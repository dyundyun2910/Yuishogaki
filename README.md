...
- 📋 **詳細表示**: マーカークリックで寺社の詳細情報をモーダル表示 ✅
- 🖼️ **画像スライダー**: 複数の由緒書き写真をスライドショー形式で閲覧 ✅
- 🔍 **全文検索**: 寺社名や由緒書きの内容で検索可能 (実装予定)
...
3. EXIF情報から位置情報を抽出:
   ...
   ```
4. Gemini AIで画像から寺社情報を取得（JSON形式で出力）
5. 取得したJSONを `public/data/temples-from-gemini.json` として保存
6. 2つのJSONをマージ:
   ```bash
